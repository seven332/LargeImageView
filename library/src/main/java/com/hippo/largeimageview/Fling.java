...
class Fling {
    ...
    public static final Interpolator FLING_INTERPOLATOR = new Interpolator() {
        @Override
        public float getInterpolation(float input) {
            final int index = (int) (NB_SAMPLES * input);
            float distanceCoef = 1.f;
            final float velocityCoef;
            if (index < NB_SAMPLES) {
    ...
    protected int getSplineFlingDuration(float velocity) {
        final double l = getSplineDeceleration(velocity);
        final double decelMinusOne = DECELERATION_RATE - 1.0;
        return (int) (1000.0 * Math.exp(l / decelMinusOne));
    }

    protected double getSplineFlingDistance(float velocity) {
        final double l = getSplineDeceleration(velocity);
        final double decelMinusOne = DECELERATION_RATE - 1.0;
     ...
     **/
    protected int adjustDuration(float start, float oldFinal, float newFinal, int duration) {
